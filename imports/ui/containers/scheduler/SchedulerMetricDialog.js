  ...
  _onAddMetric = selected => {
    ...
    Meteor.call('scheduler.addMetricToQuarter', {
      ...
      if (error) {
        ...
        onDismiss();
        Notifications.success({message: 'Added'});
    ...
    return ( < SkyLightStateless isVisible={ show }
        ...
        <div className="row">
          {METRICS_UI_INFO.map((m, key) => (
            <div key={key} className="col-lg-4 col-md-6 col-sm-12 col-xs-12" style={{marginBottom: 20}}
                 onClick={() => !this.isMetricDisabled(m) ? this._onAddMetric(m) : null}>
              <div
                className={this.isMetricDisabled(m) ? 'scheduler-metric-dialog__item disabled' : 'scheduler-metric-dialog__item enable'}>
